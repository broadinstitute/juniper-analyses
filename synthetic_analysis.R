...
plot_params <- function(ideal = T, output_plot = T){
  ...
  for (p in params) {
    ...
    for (i in 1:nrow(combos)) {
      dir <- paste0("synthetic-outbreaks/experiment_", i)
    ...
    if(run == "ideal"){
      ...
      if(i <= 21){
        net <- readLines(paste0(
          "synthetic-outbreaks/experiment_", i, "/badtrip/summary_network.txt"
        ))
        # Get number of hosts
      ...
      load(paste0(
        "synthetic-outbreaks/experiment_",
        i,
...
message("---Donor Recipient Accuracy---")
for (m in models) {
  message(
    paste0(
      "The donor-recipient accuracy based on direct transmissions only for ",
      model_names[match(m, models)],
      " is: ",
      signif(mean(comp_df$Accuracy[comp_df$Model == m], na.rm = T), 3)
    )
  )
}
...
for (m in models) {
  message(
    paste0(
      "The donor-recipient accuracy based on direct and indirect transmissions for ",
      model_names[match(m, models)],
      " is: ",
      signif(mean(comp_df$Accuracy_Indirect[comp_df$Model == m], na.rm = T), 3)
    )
  )
}

# Direct donor set accuracy
message("---Donor Set Accuracy---")
for (m in models) {
  message(
    paste0(
      "The donor set accuracy based on direct transmissions only for ",
      model_names[match(m, models)],
      " is: ",
      signif(mean(comp_df$Coverage[comp_df$Model == m], na.rm = T), 3)
    )
  )
}
...
for (m in models) {
  message(
    paste0(
      "The donor set accuracy based on direct and indirect transmissions for ",
      model_names[match(m, models)],
      " is: ",
      signif(mean(comp_df$Coverage_Indirect[comp_df$Model == m], na.rm = T), 3)
    )
  )
}

# Direct most likely donor accuracy
message("---Most Likely Donor Accuracy---")
for (m in models) {
  message(
    paste0(
      "The most likely donor accuracy based on direct transmissions only for ",
      model_names[match(m, models)],
      " is: ",
      signif(mean(comp_df$Hit[comp_df$Model == m], na.rm = T), 3)
    )
  )
}
...
for (m in models) {
  message(
    paste0(
      "The most likely donor accuracy based on direct and indirect transmissions for ",
      model_names[match(m, models)],
      " is: ",
      signif(mean(comp_df$Hit_Indirect[comp_df$Model == m], na.rm = T), 3)
    )
  )
}
...
comp_df1 <- suppressMessages(reshape2::melt(comp_df1))
comp_df2 <- suppressMessages(reshape2::melt(comp_df2))
...
bt_dens <- function(param, plot = TRUE){
  ...
  for (i in 1:nrow(combos)) {
    if(i <= 21){
      bt_log <- read.table(paste0("synthetic-outbreaks/experiment_", i, "/badtrip/BADTRIP_setup.log"), sep = "\t", header = T)
...
message("---Evolution Rate Estimation---")
message(paste0(
  "The RMSE for estimating the evolution rate (subs/site/day) with JUNIPER is: ",
  signif(mu_rmse_juniper, 3)
))

message(
  paste0(
    "The percent increase in RMSE for estimating the evolution rate (subs/site/day) with BadTrIP as compared to JUNIPER is: ",
    signif(100 * (sqrt(mean(mu_err$Error[mu_err$Model == "BadTrIP"]^2, na.rm = T)) - mu_rmse_juniper) / mu_rmse_juniper, 3)
  )
)

message(
  paste0(
    "The percent increase in RMSE for estimating the evolution rate (subs/site/day) with outbreaker2 as compared to JUNIPER is: ",
    signif(100 * (sqrt(mean(mu_err$Error[mu_err$Model == "outbreaker2"]^2)) - mu_rmse_juniper) / mu_rmse_juniper, 3)
  )
)
...
message("---Sampling Probability Estimation---")
message(paste0(
  "The RMSE for estimating the sampling probability with JUNIPER is: ",
  signif(pi_rmse_juniper, 3)
))

message(
  paste0(
    "The percent increase in RMSE for estimating the sampling probability with TransPhylo as compared to JUNIPER is: ",
    signif(100 * (sqrt(mean(pi_err$Error[pi_err$Model == "TransPhylo"]^2)) - pi_rmse_juniper) / pi_rmse_juniper, 3)
  )
)

message(
  paste0(
    "The percent increase in RMSE for estimating the sampling probability with outbreaker2 as compared to JUNIPER is: ",
    signif(100 * (sqrt(mean(pi_err$Error[pi_err$Model == "outbreaker2"]^2, na.rm = T)) - pi_rmse_juniper) / pi_rmse_juniper, 3)
  )
)
...
message("---Reproductive Number Estimation---")
message(paste0(
  "The RMSE for estimating the reproductive number with JUNIPER is: ",
  signif(R_rmse_juniper, 3)
))

message(
  paste0(
    "The percent increase in RMSE for estimating the reproductive number with TransPhylo as compared to JUNIPER is: ",
    signif(100 * (sqrt(mean(R_err$Error[R_err$Model == "TransPhylo"]^2, na.rm = T)) - R_rmse_juniper) / R_rmse_juniper, 3)
  )
)

## Plot of runtime

# Duration of run for Juniper
juniper_dur <- c()
juniper_ess <- c()
for (i in 1:23) {
  load(paste0(
    "synthetic-outbreaks/experiment_",
    i,
    "/runtime/juniper_ideal.RData"
  ))
  juniper_dur[i] <- runtime

  load(paste0("synthetic-outbreaks/experiment_", i, "/output_ideal.RData"))
...
for (i in 1:23) {
  ...
  out <- read.table(paste0("synthetic-outbreaks/experiment_", i, "/badtrip/BADTRIP_setup.log"), sep = "\t", header = T)
...
for (i in 1:23) {
  ...
  load(paste0("synthetic-outbreaks/experiment_", i, "/runtime/tp.RData"))
  load(paste0("synthetic-outbreaks/experiment_", i, "/res_TransPhylo_slim.RData"))
...
message(paste0(
  "In the presence of any ambiguous sites, the average runtime per 100 effective samples for BadTrIP is: ",
  signif(e_dur_per_ess * 100 / 24, 3),
  " days"
))
...
message(paste0(
  "The average runtime per 100 effective samples for JUNIPER is: ",
  signif(mean(juniper_dur * 100 / juniper_ess), 3),
  " hours"
))

message(paste0(
  "The average runtime per 100 effective samples for BadTrIP (assuming no ambiguous sites) is: ",
  signif(mean(badtrip_dur * 100 / badtrip_ess, na.rm = T), 3),
  " hours"
))

message(paste0(
  "The average runtime per 100 effective samples for outbreaker2 is: ",
  signif(mean(o2_dur * 100 / o2_ess), 3),
  " hours"
))

message(paste0(
  "The average runtime per 100 effective samples for TransPhylo is: ",
  signif(mean(tp_dur * 100 / tp_ess), 3),
  " hours"
))
...
dur_df <- data.frame(
  ...
  Duration = c(o2_dur * 100 / o2_ess, tp_dur * 100 / tp_ess, juniper_dur * 100 / juniper_ess, badtrip_dur * 100 / badtrip_ess),
...
message("Figures were generated successfully!")
