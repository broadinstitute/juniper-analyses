...
Rcpp::sourceCpp("src/cpp_subroutines.cpp")

# Set to TRUE to re-generate SARS-CoV-2 transmission analysis; otherwise use pre-computed results
...
if(regenerate){
  ...
  save(p_trans_boosted, file = "mass-10k/p_trans_boosted.RData")
  save(p_trans_vax, file = "mass-10k/p_trans_vax.RData")
  save(p_trans_unvax, file = "mass-10k/p_trans_unvax.RData")
}else{
  load("mass-10k/mean_trans_boosted.RData")
  load("mass-10k/mean_trans_vax.RData")
  load("mass-10k/mean_trans_unvax.RData")

  load("mass-10k/p_trans_boosted.RData")
  load("mass-10k/p_trans_vax.RData")
  load("mass-10k/p_trans_unvax.RData")
}
...
message(paste0(
  "Mean number of transmissions per boosted individual: ",
  signif(mean(vax_df$Transmissions[vax_df$Status == "Boosted"]), 3)
))

message(paste0(
  "Mean number of transmissions per vaccinated individual: ",
  signif(mean(vax_df$Transmissions[vax_df$Status == "Vaccinated"]), 3)
))

message(paste0(
  "Mean number of transmissions per unvaccinated individual: ",
  signif(mean(vax_df$Transmissions[vax_df$Status == "Unvaccinated"]), 3)
))
...
message(paste0(
  "Effective sample size for the SARS-CoV-2 dataset based on the log-posterior: ",
  signif(coda::effectiveSize(llik[(n_iters * 0.5 + 1):n_iters]), 3)
))

for (i in 1:length(param_names)) {
  message(paste0(
    "Effective sample size for the SARS-CoV-2 dataset based on the ",
    param_names[i],
    ": ",
    signif(coda::effectiveSize(params[[i]][(n_iters * 0.5 + 1):n_iters]), 3)
  ))
}
...
message(paste0(
  "Effective sample size for the H5N1 dataset based on the log-posterior: ",
  signif(coda::effectiveSize(llik[(n_iters * 0.5 + 1):n_iters]), 3)
))

for (i in 1:length(param_names)) {
  message(paste0(
    "Effective sample size for the H5N1 dataset based on the ",
    param_names[i],
    ": ",
    signif(coda::effectiveSize(h5n1_params[[i]][(n_iters * 0.5 + 1):n_iters]), 3)
  ))
}
...
# Reproductive number
message(paste0(
  "Overall reproductive number, H5N1: ",
  signif(mean(h5n1_params[[4]][(n_iters * 0.5 + 1):n_iters]), 3),
  " (95% HPD: ",
  paste(signif(quantile(h5n1_params[[4]][(n_iters * 0.5 + 1):n_iters], c(0.025, 0.975)), 3), collapse = "-"),
  ")"
))

# Reproductive number
message(paste0(
  "Reproductive number in California, H5N1: ",
  signif(mean(h5n1_trans_df$Offspring[h5n1_trans_df$State == "California"]), 3),
  " (95% HPD: ",
  paste(signif(quantile(h5n1_trans_df$Offspring[h5n1_trans_df$State == "California"], c(0.025, 0.975)), 3), collapse = "-"),
  ")"
))
