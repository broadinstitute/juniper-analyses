...
if(regenerate){
  ...
  init <- initialize(
    ...
    init_mu = 3e-6,
    n_global = 1000,
    root = "G_BRSVSweden620p4_S12-cons",
    a_g = 7,
    ongoing = F
  )
...
dates <- as.Date(read.csv("brsv-outbreak/input_data/metadata.csv")$date)
...
fasta <- read.FASTA("brsv-outbreak/input_data/aligned.fasta")
ref <- fasta[1]
fasta <- fasta[-1] # Getting rid of root
...
# We manually construct the phylo tree here due to consistent errors encountered in using IQ-TREE and TreeTime for this particular dataset
edges <- matrix(
  c(
    9, 1,
    9, 5,
...
## BadTrIP
fasta <- read.FASTA("brsv-outbreak/input_data/aligned.fasta")[-1]
names <- gsub("\\|.*", "", names(fasta))
...
if(regenerate){
  ...
  no_root <- fasta[-1]
  dates <- as.Date(gsub(".*\\|", "", names(no_root)))
  names <- gsub("\\|.*", "", names(no_root))
  names(no_root) <- names
  write.FASTA(no_root, "south-africa-outbreak/input_data/no_root.fasta")

  if(dir.exists("south-africa-outbreak/iqtree")){
    unlink("south-africa-outbreak/iqtree", recursive = TRUE)
    dir.create("south-africa-outbreak/iqtree")
  }

  if(dir.exists("south-africa-outbreak/treetime")){
    unlink("south-africa-outbreak/treetime", recursive = TRUE)
  }

  # Run IQTree
  system(
    "iqtree2 -s south-africa-outbreak/input_data/no_root.fasta -m JC --prefix south-africa-outbreak/iqtree/iqtree-results"
  )
  ...
  # Run TreeTime
  system(
    "treetime --tree south-africa-outbreak/iqtree/iqtree-results.treefile --dates south-africa-outbreak/iqtree/date.csv --aln south-africa-outbreak/input_data/no_root.fasta --outdir south-africa-outbreak/treetime --coalescent skyline --n-skyline 2 --greedy-resolve"
  )
...
# Names according to transphylo
ref <- read.FASTA("south-africa-outbreak/input_data/aligned.fasta")[1]
tp_names <- c(names(ref), res_TransPhylo[[1]]$ctree$nam)
tp_names <- gsub("\\|.*", "", tp_names)
n <- length(tp_names)
